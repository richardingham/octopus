...
class machine_declaration(Block):
    ...
    def _run(self):
        ...
        def _connect():
            ...
            if connection is None:
                raise Exception(
                    "No connection specified for machine '{:s}'".format(
                        self.fields["NAME"]
                    )
                )
            ...
            self.machine = cls(
                connection, alias=self.fields["NAME"], **self.getMachineParams()
            )
            ...
            yield defer.gatherResults(
                [task.deferLater(reactor, 2, lambda: result), self.machine.reset()]
            )
    ...
    def getGlobalDeclarationNames(self):
        ...
        return Block.getGlobalDeclarationNames(
            self, [name] if not self.disabled else []
        )
...
class machine_mt_icir(machine_declaration):
    ...
    def getMachineParams(self):
        ...
        try:
            return {"stream_names": json.loads(self.mutation)["stream_names"]}
...
class machine_phidgets_phsensor(machine_declaration):
    ...
    def getMachineParams(self):
        ...
        try:
            return {"min_change": float(json.loads(self.mutation)["min_change"])}
        except (ValueError, KeyError):
            return {}
...
class connection_tcp(connection_declaration):
    def eval(self):
        return defer.succeed(
            octopus.transport.basic.tcp(
                str(self.fields["HOST"]), int(self.fields["PORT"])
            )
        )
...
class connection_serial(connection_declaration):
    def eval(self):
        return defer.succeed(
            octopus.transport.basic.serial(
                str(self.fields["PORT"]), baudrate=int(self.fields["BAUD"])
            )
        )
