...
def _getFloatArg(request, arg):
    return _getArg(request, arg, float, 0.0)
...
class Root(resource.Resource):
    def render_GET(self, request):
        saved_sketches = sketch.find(
            default_search={"deleted": {"value": 0}},
            order=[{"column": "modified_date", "dir": "desc"}],
        ...
        past_experiments = experiment.find(
            ...
            order=[{"column": "finished_date", "dir": "desc"}],
...
class SketchFind(resource.Resource):
    def render_GET(self, request):
        ...
        sketch.find(filters, sorts, start, limit).addCallback(
            _respondWithJSON, request
...
class NewSketch(resource.Resource):
    ...
    def render_POST(self, request: http.Request):
        ...
        sketch.Sketch.createId().addCallback(_redirect).addErrback(_error, request)
...
class CopySketch(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        sketch.Sketch.createId().addCallback(_copy).addErrback(_error, request)
...
class DeleteSketch(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        sketch.Sketch.delete(self._id).addCallback(
            _redirectOrJSON, request, redirect_url, {"deleted": str(self._id)}
        ).addErrback(_error, request)
...
class UndeleteSketch(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        sketch.Sketch.restore(self._id).addCallback(
            _redirectOrJSON, request, redirect_url, {"restored": str(self._id)}
        ).addErrback(_error, request)
...
class ExperimentFind(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        experiment.find(
            ...
            start,
            limit,
            {"deleted": {"value": 0}, "finished_date": {"value": 0, "operator": "gt"}},
...
class ShowExperiment(resource.Resource):
    ...
    def render_GET(self, request):
        def _done(exists):
            ...
            expt = next(
                (expt for expt in running_experiments() if expt.id == self._id), None
            )
...
class GetExperimentData(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        expt.loadData(variables, start, end).addCallback(
            _respondWithJSON, request
        ).addErrback(_error, request)
...
class DownloadExperimentData(resource.Resource):
    ...
    def _render_GET(self, request):
        ...
        tpl = template.ExperimentDownload(experiment.CompletedExperiment(self._id))
        ...
        try:
            ...
            filename = ".".join(
                [
                    ...
                    time.strftime("%Y%m%d_%H%M%S", time.gmtime(expt.finished_date)),
                    "xlsx",
                ]
            )
...
class DeleteExperiment(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        experiment.Experiment.delete(self._id).addCallback(
            _redirectOrJSON, request, redirect_url, {"deleted": str(self._id)}
        ).addErrback(_error, request)
...
class UndeleteExperiment(resource.Resource):
    ...
    def render_POST(self, request):
        ...
        experiment.Experiment.restore(self._id).addCallback(
            _redirectOrJSON, request, redirect_url, {"restored": str(self._id)}
        ).addErrback(_error, request)
...
def makeConsoleServerFactory():
    ...
    factory.publicKeys[b"ssh-rsa"] = keys.Key.fromFile(
        os.path.join(data_path, "ssh-keys", "ssh_host_rsa_key.pub")
    )
    factory.privateKeys[b"ssh-rsa"] = keys.Key.fromFile(
        os.path.join(data_path, "ssh-keys", "ssh_host_rsa_key")
    )
...
def makeService(options):
    ...
    ws_factory = makeWebsocketServerFactory(
        str(options["wshost"]), int(options["wsport"])
    )
    ...
    try:
        ...
        internet.TCPServer(
            int(options["consoleport"]), console_factory
