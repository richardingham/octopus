...
class Sketch(EventEmitter):
    ...
    def delete(cls, id):
        return cls.db.runOperation(
            "UPDATE sketches SET deleted = 1 WHERE guid = ?", (id,)
        )

    @classmethod
    def restore(cls, id):
        return cls.db.runOperation(
            "UPDATE sketches SET deleted = 0 WHERE guid = ?", (id,)
        )
    ...
    def _loadFrom(self, id, copy=False):
        sketch = yield self.db.runQuery(
            "SELECT title FROM sketches WHERE guid = ?", (id,)
        )
    ...
    def close(self):
        ...
        if self._eventIndex > self._snapEventIndex:
            # Write a snapshot
            snapFile = self._sketchDir.child(
                "snapshot." + str(self._eventIndex) + ".log"
            )
    ...
    def rename(self, title):
        ...
        self.db.runOperation(
            "UPDATE sketches SET title = ? WHERE guid = ?", (title, self.id)
        )
        self.title = title
    ...
    def runExperiment(self, context):
        ...
        self.notifySubscribers(
            "experiment",
            "state-started",
            {"sketch": self.id, "experiment": self.experiment.id},
            self.experiment,
        )
    ...
    def pauseExperiment(self, context):
        ...
        self.log.info("Pausing experiment for sketch {log_source.id!s}")
    ...
    def resumeExperiment(self, context):
        ...
        self.log.info("Resuming experiment for sketch {log_source.id!s}")
    ...
    def stopExperiment(self, context):
        ...
        self.log.info("Stopping experiment for sketch {log_source.id!s}")
    ...
    def renameSketch(self, payload, context):
        ...
        self.notifySubscribers(
            "sketch",
            "renamed",
            {"event": self._eventIndex, "title": payload["title"]},
            context,
        )
    ...
    def processEvent(self, event, context):
        ...
        self.notifySubscribers(
            event.jsProtocol, event.jsTopic, event.valuesWithEventId(eid), context
        )
    ...
    def _writeEvent(self, eventType, data):
        ...
        event = {"index": self._eventIndex, "type": eventType, "data": data}
