...
def _subclasses(cls):
    return cls.__subclasses__() + [
        g for s in cls.__subclasses__() for g in _subclasses(s)
    ]
...
def get_block_plugin_modules():
    ...
    return {
        ...
        for finder, name, ispkg in iter_namespace(octopus.blocks)
    }
...
class Workspace(Runnable, Pausable, Cancellable, EventEmitter):
    ...
    def addBlock(self, id, type, fields=None, x=0, y=0):
        ...
        try:
            ...
            self.log.warn(
                "Unknown block type {block_type} requested", block_type=blockType
            )
    ...
    def _run(self):
        ...
        def _checkFinished(error=None):
            self.log.debug(
                "Finished?: Waiting for {count} blocks", count=len(runningBlocks)
            )
            ...
            for dep in deps:
                if dep not in allDeclaredGlobalVariables:
                    self.emit(
                        "log-message",
                        level="error",
                        message="Referenced variable {:s} is never defined. ".format(
                            dep
                        ),
                        block=block.id,
                    )
        ...
        if len(blocksToRunImmediately) == 0:
            self.emit("log-message", level="error", message="No blocks can run.")
            dependencyError = True
        ...
        if len(circularDeps) > 0:
            self.emit(
                "log-message", level="error", message="Circular dependencies detected:"
            )

            for item in sorted(circularDeps, key=lambda item: item["position"]):
                self.emit(
                    "log-message",
                    level="error",
                    message="* {:s} depends on {:s}".format(
                        ", ".join(item["decls"]), ", ".join(item["deps"])
                    ),
                    block=item["block"],
                )
        ...
        # Blocks are sorted first by x then by y.
            ...
            for block in sorted(blocksToRunImmediately, key=lambda b: b.position):
                _runBlock(block)
...
class Variables(EventEmitter):
    ...
    def remove(self, name):
        ...
        if isinstance(variable, BaseVariable):
            variable.off("change", self._handlers[name])
            self.emit("variable-removed", name=name, variable=variable)

        elif isinstance(variable, Component):
            for attrname, attr in variable.variables.items():
                attr.off("change", self._handlers[name][attrname])
    ...
    def rename(self, oldName, newName):
        ...
        for name in oldNames:
            ...
            self.emit(
                "variable-renamed", oldName=name, newName=newName, variable=variable
            )
...
class Block(BaseStep, EventEmitter):
    ...
    def emitLogMessage(self, message, level):
        self.workspace.emit("log-message", level=level, message=message, block=self.id)

    def connectNextBlock(self, childBlock):
        if self.nextBlock is not None:
            raise Exception(
                "Block.connectNextBlock (#%s): parent #%s already has a next Block"
                % (childBlock.id, self.id)
            )
        if childBlock.prevBlock is not None:
            raise Exception(
                "Block.connectNextBlock (#%s): child #%s already has a previous Block"
                % (self.id, childBlock.id)
            )
        ...
        if self.state in (State.RUNNING, State.PAUSED):
            ...
            if anyOfStackIs(childBlock, [State.RUNNING, State.PAUSED]):
                if childBlock._complete is not None:
                    self._complete = defer.Deferred()
                    childBlock._complete.addCallbacks(
                        self._complete.callback, self._complete.errback
                    )
    ...
    def disconnectNextBlock(self, childBlock):
        if self.nextBlock != childBlock:
            raise Exception(
                "Block.disconnectNextBlock: must pass the correct child block"
            )
    ...
    def run(self, parent=None):
        ...
        def _done(result=None):
            ...
            if self.state is State.ERROR or self._complete is None:
                ...
                d.addErrback(_disconnected).addCallbacks(
    ...
    def reset(self, propagate=True):
        ...
        if (not propagate and self.state in (State.RUNNING, State.PAUSED)) or (
            propagate and anyOfStackIs(self, (State.RUNNING, State.PAUSED))
        ):
            raise AlreadyRunning
        ...
        if self.state is not State.READY:
            ...
            for block in self.inputs.values():
                try:
                    ...
                    results.append(
                        block.cancel(propagate=True).addCallback(lambda _: block.reset)
                    )
                except AttributeError:
                    pass
    ...
    def toEvents(self):
        ...
        events.append(
            {
                "type": "AddBlock",
                "data": {"id": self.id, "type": self.type, "fields": self.fields},
            }
        )
        ...
        if self.mutation != "":
            events.append(
                {
                    "type": "SetBlockMutation",
                    "data": {"id": self.id, "mutation": self.mutation},
                }
            )
        ...
        if self.comment != "":
            events.append(
                {
                    "type": "SetBlockComment",
                    "data": {"id": self.id, "value": self.comment},
                }
            )
            ...
            events.append(
                {
                    ...
                    "data": {
                        ...
                        "parent": self.outputBlock.id,
                        "input": self.parentInput,
                    },
                }
            )
                ...
                events.append(
                    {
                        ...
                        "data": {
                            "id": self.id,
                            "connection": "input-statement",
                            "parent": self.prevBlock.id,
                            "input": self.parentInput,
                        },
                    }
                )
        ...
        if self.disabled:
            events.append(
                {"type": "SetBlockDisabled", "data": {"id": self.id, "value": True}}
            )

        if self.inputsInline is False:
            events.append(
                {
                    "type": "SetBlockInputsInline",
                    "data": {"id": self.id, "value": False},
                }
            )
        ...
        if self.collapsed:
            events.append(
                {"type": "SetBlockCollapsed", "data": {"id": self.id, "value": True}}
            )
...
class Event(object):
    ...
    def toJSON(self):
        ...
        return json.dumps({"type": self.type, "data": self.values})
...
class SetBlockPosition(Event):
    ...
    def apply(self, workspace):
        ...
        block.position = [int(self.values["x"] or 0), int(self.values["y"] or 0)]
