...
class BlobMaker:
    ...
    def extract(
        ...
        blobs = self.extractFromBinary(
            img.binarize(threshval, 255, threshblocksize, threshconstant).invert(),
            img,
            minsize,
            maxsize,
        )
    ...
    def extractFromBinary(
        self, binaryImg, colorImg, minsize=5, maxsize=-1, appx_level=3
    ):
        """
        ...
        * *appx_level* - The blob approximation level - an integer for the maximum distance between the true edge and the approximation edge - lower numbers yield better approximation.
        ...
        ptest = (4 * 255.0) / (
            binaryImg.width * binaryImg.height
        ...
        try:
            ...
            retVal = self._extractFromBinary(
                seq, False, colorImg, minsize, maxsize, appx_level
            )
    ...
    def _extractFromBinary(self, seq, isaHole, colorImg, minsize, maxsize, appx_level):
        ...
        while True:
            if (
                not isaHole
            ):  # if we aren't a hole then we are an object, so get and return our featuress
        ...
        for nextLayer in nextLayerDown:
            retVal += self._extractFromBinary(
                nextLayer, not isaHole, colorImg, minsize, maxsize, appx_level
            )
            ...
            try:
                ...
                if retVal.mContour is not None:
                    ...
                    appx = cv2.approxPolyDP(
                        np.array([retVal.mContour], "float32"), appx_level, True
                    )
                    for p in appx:
    ...
    def _getMask(self, seq, bb):
        ...
        cv.Zero(mask)
        cv.DrawContours(
            mask, seq, (255), (0), 0, thickness=-1, offset=(-1 * bb[0], -1 * bb[1])
        )
        holes = seq.v_next()
        if holes is not None:
            cv.DrawContours(
    ...
    def _getHullMask(self, hull, bb):
        ...
        cv.Zero(mask)
        cv.DrawContours(
            mask, hull, (255), (0), 0, thickness=-1, offset=(-1 * bb[0], -1 * bb[1])
        )
        return mask
