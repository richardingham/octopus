...
class PHD2000Infuser(Machine):
    ...
    def setup(self, syringe_diameter):
        ...
        def set_rate(rate):
            if rate < 0.0001:
                ...
                result, status = yield self.protocol.write(
                    self.pumpNumber + "RAT {:.4f}".format(rate)[:10] + " UM",
                )
        ...
        def set_target(target):
            ...
            # Set the property
            result, status = yield self.protocol.write(
                self.pumpNumber + "TGT {:.4f}".format(target)[:10]
            )
        ...
        self.rate = Property(
            title="Flow rate", type=float, unit="uL/min", setter=set_rate
        )
        self.target_volume = Property(
            title="Target Volume", type=float, unit="mL", setter=set_target
        )
        self.dispensed = Stream(title="Volume dispensed", type=float, unit="mL")

        self.ui = ui(traces=[], properties=[self.rate, self.dispensed])
    ...
    def start(self):
        ...
        def monitor():
            return defer.gatherResults(
                [
                    self.protocol.write(self.pumpNumber + "RAT")
                    .addCallback(interpret_rate)
                    .addErrback(log.err),
                    self.protocol.write(self.pumpNumber + "DEL")
                    .addCallback(interpret_delivered)
                    .addErrback(log.err),
                    self.protocol.write(self.pumpNumber + "TGT")
                    .addCallback(interpret_target)
                    .addErrback(log.err),
                ]
            )
