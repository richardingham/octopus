...
class _heater(Component):
    def __init__(self, set_power, set_temp, set_delta):
        self.power = Property(
            title="Hotplate Power", type=str, options=("on", "off"), setter=set_power
        )
        self.target = Property(
            title="Hotplate Target Temperature", type=int, unit="C", setter=set_temp
        )
        self.safetydelta = Property(
            title="Hotplate Safety Delta", type=int, unit="C", setter=set_delta
        )

        self.mediumtemp = Stream(title="Medium Temperature", type=float, unit="C")
        self.mediumsafetytemp = Stream(
            title="Medium (Safety) Temperature", type=float, unit="C"
        )
        self.hotplatetemp = Stream(title="Hotplate Temperature", type=float, unit="C")
        self.hotplatesafetytemp = Stream(
            title="Hotplate (Safety) Temperature", type=float, unit="C"
        )
...
class _stirrer(Component):
    def __init__(self, set_power, set_speed):
        self.power = Property(
            title="Stirrer Power", type=str, options=("on", "off"), setter=set_power
        )
        self.target = Property(
            title="Stirrer Target Speed", type=int, unit="rpm", setter=set_speed
        )
...
class HeiEnd(Machine):
    ...
    def setup(self):
        ...
        # setup variables
        self.heater = _heater(
            _set_heater_power(self), _set_heater_target(self), _set_heater_delta(self)
        )
    ...
    def start(self):
        ...
        addMonitor(
            "IN_PV_1", lambda x: float(x) if x != 999.9 else 0, self.heater.mediumtemp
        )
        addMonitor(
            "IN_PV_2",
            lambda x: float(x) if x != 999.9 else 0,
            self.heater.mediumsafetytemp,
        )
