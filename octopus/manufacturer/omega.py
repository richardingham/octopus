...
# Package Imports
from octopus.machine import Machine, Stream, ui
...
class HH306A(Machine):
    ...
    def setup(self):
        ...
        self.ui = ui(traces=[], properties=[self.temp1, self.temp2])
    ...
    def start(self):
        ...
        model_no = yield self.protocol.write(
            "K", length=3, start_delimiter="", end_delimiter="\r"
        )

        if model_no != "306":
            raise Exception(
                "HH306A: Expected model '306', received '{:s}'".format(model_no)
            )
        ...
        def monitor():
            return (
                self.protocol.write("A", length=8)
                .addCallback(interpret_data)
                .addErrback(log.err)
            )
