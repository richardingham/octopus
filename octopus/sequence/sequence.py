...
class Step(util.BaseStep, EventEmitter):
    ...
    def __cb(self, cb_fn, dep_fn, value):
        ...
        try:
            dep_fn().addErrback(log.err).addBoth(cb).addErrback(log.err)
        except NotRunning:
            pass
...
class _StepWithChild(Step):
    ...
    def _reset(self):
        return defer.gatherResults(
            [defer.maybeDeferred(Step._reset, self), self._step.reset()]
        )

    def _pause(self):
        return defer.gatherResults(
            [defer.maybeDeferred(Step._pause, self), self._step.pause()]
        )

    def _resume(self):
        return defer.gatherResults(
            [defer.maybeDeferred(Step._resume, self), self._step.resume()]
        )
        ...
        def advance(result=None):
            if self.state is State.PAUSED:
                ...
                try:
                    ...
                    reactor.callLater(
                        0,
                        lambda step: step.run(parent=self).addCallbacks(
                            advance, self._error
                        ),
                        step,
                    )
...
class TickStep(Step):
    ...
    def __init__(
        self, expr, fn, interval, now=True, max_calls=None, fnArgs=None, fnKeywords=None
    ):
        Step.__init__(self)

        self._tick = util.Trigger(
            expr, fn, interval, now, max_calls, fnArgs, fnKeywords
        )
