    ...
    def _iterate(self):
        ...
        try:
            if self.state is State.PAUSED:
                ...
                and self._max_calls > 0
                and self._calls >= int(self._max_calls)
            ):
                raise StopIteration
...
class Tick(Caller, Looping, Dependent):
    ...
    def __init__(
        self, fn, interval, now=True, max_calls=None, fnArgs=None, fnKeywords=None
    ):
        """
        Initialise a Tick.
    ...
    def _reset(self):
        ...
        for d in self._dependents:
            try:
                ...
                r.append(
                    d.cancel().addCallback(lambda _: d.reset())
                )  # May take a while - should return a deferred. So all these functions should return deferred's (like run?)
