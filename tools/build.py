...
def build_machine_block_definition_js(filename):
    ...
    with open(filename, "w") as fp:
        ...
        for name, definition in workspace.get_machine_js_definitions():
            fp.write(
                f"Blockly.Blocks.addMachineBlock('{name}', {json.dumps(definition)});\n"
            )
...
def build_connection_block_definition_js(filename):
    ...
    with open(filename, "w") as fp:
        ...
        for name, definition in workspace.get_connection_js_definitions():
            fp.write(
                f"Blockly.Blocks.addConnectionBlock('{name}', {json.dumps(definition)});\n"
            )
...
def fetch_resource(url, filename, allow_fail=False):
    ...
    try:
        downloaded_file = wget.download(url=url, out=cache_file)
        print("\n")
    except HTTPError:
        if allow_fail:
            print("  [Not found]")
...
if __name__ == "__main__":
    ...
    build_machine_block_definition_js(
        os.path.join(
            blocktopus_dir, "resources", "blockly", "pack", "octopus-machines.js"
        )
    )

    print("Building connection blocks definition JS")
    build_connection_block_definition_js(
        os.path.join(
            blocktopus_dir, "resources", "blockly", "pack", "octopus-connections.js"
        )
    )
    ...
    with open(resources_json) as templates_file:
        ...
        for cache_filename, resource_url in resources.items():
            ...
            if (
                len(split_filename) > 3
                and split_filename[-2] == "min"
                and split_filename[-1] in ("js", "css")
            ):
