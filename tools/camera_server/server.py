...
def _get_image_format(frame) -> dict:
    image_format = dict(
        height=frame.shape[0], width=frame.shape[1], colorspace=COLORSPACE_BGR
    )
        ...
        try:
            ...
            if command == "F":
                return self.send_data(
                    b"F",
                    json.dumps(_get_image_format(_get_image(camera_id))).encode(
                        "utf-8"
                    ),
                )
...
async def tcp_server(app: web.Application):
    ...
    server = await loop.create_server(lambda: CameraServerProtocol(), "127.0.0.1", 8081)
